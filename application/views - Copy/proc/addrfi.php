...
<div class="row">
    <div class="col-md-12">
      ...
      <!-- /.box-header -->
    <form class="form-horizontal" action="<?php echo base_url(); ?>proc/rfi/save<?php if (isset($info)) echo "/$info->rfi_id"; ?>" method="POST" enctype="multipart/form-data" onsubmit="return formsubmit();">
         <div class="box-body">
         <div class="form-group">
         <label class="col-sm-2 control-label">RFI Date <span style="color:red;">  *</span></label>
         <div class="col-sm-2">
               <div class="input-group date">
             <div class="input-group-addon">
               <i class="fa fa-calendar"></i>
             </div>
             <input type="text" name="rfi_date" readonly id="rfi_date" class="form-control pull-right" value="<?php if(isset($info)) echo findDate($info->rfi_date); else echo date('d/m/Y'); ?>">
           </div>
           <span class="error-msg"><?php echo form_error("rfi_date");?></span>
          </div>
        ...
        <div class="form-group">
           ...
           <div class="col-sm-3">
            <select class="form-control" name="pr_type" id="pr_type">
              <option value="2"
                <?php if(isset($info)) echo '2'==$info->pr_type? 'selected="selected"':0; else echo set_select('pr_type','2');?>>Safety Stock</option>
              <option value="1"
                <?php if(isset($info)) echo '1'==$info->pr_type? 'selected="selected"':0; else echo set_select('pr_type','1');?>>Asset</option>
            </select>
      ...
      <label class="col-sm-2 control-label" style="margin-top: 14px">SCAN CODE or Search 搜索Item<span style="color:red;">  </span></label>
		...
		<tbody>
		 ...
		 $id=0;
		  if(isset($detail)):
		    foreach ($detail as  $value){
		     ...
		     $str.='<td><input type="text" name="specification[]" class="form-control" placeholder="'.$value->specification.'" value="'.$value->specification.'"  style="margin-bottom:5px;width:98%" id="specification_'  .$id. '"/> </td>';
		      ...
		      $str.= '<td><input type="text" name="quantity[]" value="'.$value->quantity.'" onblur="return checkQuantity(' .$id.');" onkeyup="return checkQuantity(' .$id.');" class="form-control"  placeholder="Quantity" style="width:60%;float:left;text-align:center"  id="quantity_' .$id. '"/></td>';
		  ...
		  <div class="col-sm-4"><a href="<?php echo base_url(); ?>proc/rfi/lists" class="btn btn-info">
...
$(document).ready(function(){
  ...
  $("#add_item").autocomplete({
        ...
        $.ajax({
            type: 'get',
            url: '<?= base_url('proc/rfi/suggestions'); ?>',
            dataType: "json",
            data: {
              term: request.term,
              pr_type: $('#pr_type').val()
        ...
        select: function (event, ui) {
           ...
           '<td> <input type="text" readonly name="product_code[]" class="form-control" placeholder="Material Code" value="'+ui.item.product_code+'" style="margin-bottom:5px;width:98%" id="product_code_' + id + '"/> </td>' +
            ...
            '<td> <input type="text" name="quantity[]" value="" onblur="return checkQuantity(' + id + ');" onkeyup="return checkQuantity(' + id + ');" class="form-control"  placeholder="Qty" style="width:98%;float:left;text-align:center"  id="quantity_' + id + '"/> </td>' +
...
////////////// end Add Item///////////////////
  ...
  if(rfi_no == ''){
      error_status=true;
      $('input[name=rfi_no]').css('border', '1px solid #f00');
    } else {
      $('input[name=rfi_no]').css('border', '1px solid #ccc');      
    }
  /////////////
  ...
  if(rfi_date == '') {
    error_status=true;
    $("#rfi_date").css('border', '1px solid #f00');
  } else {
    $("#rfi_date").css('border', '1px solid #ccc');      
  }
  if(demand_date == '') {
    error_status=true;
