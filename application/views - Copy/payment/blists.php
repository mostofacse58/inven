...
<a class="btn btn-sm btn-primary pull-right" style="margin-right:0px;" href="<?php echo base_url(); ?>payment/budget/add">
...
<!-- /.box-header -->
<div class="box-body">
  <form class="form-horizontal" action="<?php echo base_url();?>payment/budget/lists" method="GET" enctype="multipart/form-data">
  <div class="box-body">
    ...
    <label class="col-sm-2 control-label">Budget. NO </label>
      ...
      <div class="col-sm-2">
        ...
        <a class="btn btn-sm btn-primary pull-right" style="margin-right:0px;" href="<?php echo base_url(); ?>payment/budget/lists">All</a>
    </div>
  </div>
  </div>
  <!-- /.box-body -->
    ...
    <tr>
      ...
      <div class="btn-group">
        ...
        <ul class="dropdown-menu pull-right" role="menu">
          <li><a href="<?php echo base_url()?>payment/budget/view/<?php echo $row->master_id;?>">
          <i class="fa fa-eye tiny-icon"></i>View  </a></li>
        <li> <a href="<?php echo base_url()?>payment/budget/loadExcel/<?php echo $row->master_id;?>" target="_blank">
          <i class="fa fa-eye tiny-icon"></i>Excel</a></li>
        <?php if($row->status==1){ ?>
        <li> <a href="<?php echo base_url()?>payment/budget/submit/<?php echo $row->master_id;?>">
          <i class="fa fa-arrow-circle-o-right tiny-icon"></i>Submit</a></li>
        <li> <a href="<?php echo base_url()?>payment/budget/edit/<?php echo $row->master_id;?>">
          <i class="fa fa-edit tiny-icon"></i>Edit 编辑</a></li>
        <li><a href="#" class="delete" data-budgetd="<?php echo $row->master_id;?>">
...
$(".delete").click(function(e){
  job=confirm("Are you sure you want to delete this Inpaymention?");
   if(job==true){
  e.preventDefault();
  var rowId=$(this).data('budgetd');
  location.href="<?php echo base_url();?>payment/budget/deletes/"+rowId;
}else{
  return false;
}
});
