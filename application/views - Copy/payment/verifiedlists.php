...
<!-- /.box-header -->
<div class="box-body">
  <form class="form-horizontal" action="<?php echo base_url();?>payment/verified/lists" method="GET" enctype="multipart/form-data">
  <div class="box-body">
    ...
    <label class="col-sm-2 control-label">Application. NO </label>
            ...
            <select class="form-control select2"  name="status" id="status" onchange='this.form.submit()'>
              <option value="All"
                <?php  if(isset($status)) echo 'All'==$status? 'selected="selected"':0; else echo set_select('status','All');?>>
                  All</option>
              <option value="3"
                <?php  if(isset($status)) echo 3==$status? 'selected="selected"':0; else echo set_select('status',3);?>>
                  Pending</option>
              <option value="4"
      ...
      <div class="col-sm-3">
        ...
        <a class="btn btn-sm btn-primary pull-right" style="margin-right:0px;" href="<?php echo base_url(); ?>payment/verified/lists">All</a>
    </div>
  </div>
  </div>
  <!-- /.box-body -->
...
<table id="examplse1" class="table table-bordered table-striped" style="width:100%;border:#000" >
  ...
  if($lists&&!empty($lists)): $i=$page+1;
    ...
    <tr>
      ...
      <td class="text-center">
      <span class="btn btn-xs btn-<?php echo ($row->status==3||$row->status==8)?"danger":"success";?>">
        <?php 
      ...
      <div class="btn-group">
        ...
        <ul class="dropdown-menu pull-right" role="menu">
          ...
          <?php if($row->status==3){ ?>
          <li><a href="<?php echo base_url()?>payment/verified/view/<?php echo $row->payment_id;?>">
          <i class="fa fa-eye tiny-icon"></i>View & Verify</a></li>
          <?php } ?>
          <li><a href="<?php echo base_url()?>payment/verified/view/<?php echo $row->payment_id;?>">
