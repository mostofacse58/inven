...
class Checkin extends MY_Controller {
	function __construct(){
        parent::__construct();
        $this->output->nocache();
        $this->load->model('gatep/checkin_model');
     ...
     function lists(){
      ...
      if($this->input->post('perpage')!='') 
        ...
        $config['base_url']=base_url().'gatep/checkin/lists/';
        $config['suffix'] = '?' . http_build_query($_GET, '', "&");
        $config["uri_segment"] = 4;
        $config['total_rows'] = $this->checkin_model->get_count();
        ...
        //echo $data["page"]; exit();///////////
        $data['list']=$this->checkin_model->lists($config["per_page"],$data['page'] );
        ////////////////////////////////////////
        $data['heading']='Gate Pass Lists';
        $data['display']='gatep/checkinlist';
    ...
    function searchForm(){
        ...
        $data['display']='gatep/checkinsearch';
        $this->load->view('admin/master',$data);
        } else {
           redirect("logincontroller");
        }
    ...
    function search(){
       if ($this->session->userdata('user_id')) {
        $data['heading']='Ckecking Gate Pass';
        $data['info']=$this->checkin_model->checkingBarcode();
     ...
     public function approved($gatepass_id=FALSE){
        ...
        $data2['checkin_by']=$this->session->userdata('user_name');
        $data2['checkin_datetime']=date('Y-m-d h:i A');
