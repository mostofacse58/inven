...
class Lapproval extends My_Controller {
    function __construct(){
        parent::__construct();
        $this->output->nocache();
        $this->load->model('gatep/lapproval_model');
    ...
    function lists(){
      ...
      if($this->input->post('perpage')!='') $perpage=$this->input->post('perpage'); else $perpage=10;
        ...
        $config['base_url']=base_url().'gatep/lapproval/lists/';
        $config['suffix'] = '?' . http_build_query($_GET, '', "&");
        $config["uri_segment"] = 4;
        $config['total_rows'] = $this->lapproval_model->get_count();
        ...
        //echo $data["page"]; exit();///////////
        $data['list']=$this->lapproval_model->lists($config["per_page"],$data['page'] );
        ////////////////////////////////////////
        $data['heading']='Gate Pass Lists';
        $data['display']='gatep/lapprovallist';
  ...
  function viewapproved($gatepass_id){
      if ($this->session->userdata('user_id')) {
        ...
        $data['display']='gatep/lapprovalview';
        $this->load->view('admin/master',$data);
        } else {
           redirect("logincontroller");
        }
     ...
     function view($gatepass_id){
      ...
      public function approved($gatepass_id=FALSE){
        ...
        redirect("gatep/lapproval/lists");
