...
<div class="row">
    <div class="col-md-12">
      ...
      <!-- /.box-header -->
    <form class="form-horizontal" action="<?php echo base_url(); ?>aformat/requisition/save<?php if (isset($info)) echo "/$info->requisition_id"; ?>" method="POST" enctype="multipart/form-data" onsubmit="return formsubmit();">
         <div class="box-body">
...
<tbody>
 ...
 $id=0;
  if(isset($detail)):
    foreach ($detail as  $value){
       ...
       $str.='<td><input readonly type="text" name="product_code[]" class="form-control"  placeholder="Material Code" value="'.$value->product_code.'"  style="margin-bottom:5px;width:98%" id="product_item_'  .$id. '"/> </td>';
        ...
        $str.='<td><input readonly type="text" name="product_name[]" class="form-control" placeholder="'.$value->product_name.'" value="'.$value->product_name.'"  style="margin-bottom:5px;width:98%" id="product_item_'  .$id. '"/> </td>';
  ...
  <div class="col-sm-4"><a href="<?php echo base_url(); ?>aformat/requisition/lists" class="btn btn-info">
...
$(document).ready(function(){
  ...
  $("#add_item").autocomplete({
        ...
        $.ajax({
            type: 'get',
            url: '<?= base_url('aformat/requisition/suggestions'); ?>',
            dataType: "json",
            data: {
              term: request.term,
