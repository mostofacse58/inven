...
<!-- /.box-header -->
<div class="box-body">
  <form class="form-horizontal" action="<?php echo base_url();?>payment/papproved/lists" method="GET" enctype="multipart/form-data">
  <div class="box-body">
    ...
    <label class="col-sm-2 control-label">Application. NO </label>
      ...
      <div class="col-sm-3">
        ...
        <a class="btn btn-sm btn-primary pull-right" style="margin-right:0px;" href="<?php echo base_url(); ?>payment/papproved/lists">All</a>
    </div>
  </div>
  </div>
  <!-- /.box-body -->
...
<table id="examplse1" class="table table-bordered table-striped" style="width:100%;border:#000" >
  ...
  if($lists&&!empty($lists)): $i=$page+1;
    ...
    <tr>
      ...
      <td class="text-center">
      <span class="btn btn-xs btn-<?php echo ($row->status==1||$row->status==5||$row->status==8)?"danger":"success";?>">
        <?php 
        if($row->status==1) echo "Draft";
        elseif($row->status==5) echo "Pending";
      ...
      <div class="btn-group">
        ...
        <ul class="dropdown-menu pull-right" role="menu">
          <?php if($row->status==5){ ?>
          <li><a href="<?php echo base_url()?>payment/papproved/viewpahtml/<?php echo $row->payment_id;?>">
          <i class="fa fa-eye tiny-icon"></i>View & Approve</a></li>
          <?php }else{ ?>
          <li><a href="<?php echo base_url()?>payment/papproved/view/<?php echo $row->payment_id;?>">
...
$(".delete").click(function(e){
  ...
  location.href="<?php echo base_url();?>payment/applications/delete/"+rowId;
}else{
  return false;
}
});
