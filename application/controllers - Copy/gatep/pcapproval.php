...
class Pcapproval extends My_Controller {
    function __construct(){
        parent::__construct();
        $this->output->nocache();
        $this->load->model('gatep/pcapproval_model');
    ...
    function lists(){
      ...
      if($this->input->post('perpage')!='') $perpage=$this->input->post('perpage'); else $perpage=10;
        ...
        $config['base_url']=base_url().'gatep/pcapproval/lists/';
        $config['suffix'] = '?' . http_build_query($_GET, '', "&");
        $config["uri_segment"] = 4;
        $config['total_rows'] = $this->pcapproval_model->get_count();
        ...
        //echo $data["page"]; exit();///////////
        $data['list']=$this->pcapproval_model->lists($config["per_page"],$data['page'] );
        ////////////////////////////////////////
        $data['heading']='Gate Pass Lists';
        $data['display']='gatep/pcapprovallist';
  ...
  function viewapproved($gatepass_id){
      if ($this->session->userdata('user_id')) {
        ...
        $data['display']='gatep/pcapprovalview';
        $this->load->view('admin/master',$data);
        } else {
           redirect("logincontroller");
        }
     ...
     function view($gatepass_id){
      ...
      public function approved($gatepass_id=FALSE){
        $data2['approved_by3']=$this->session->userdata('user_id');
        $data2['approved_date3']=date('Y-m-d');
        ...
        redirect("gatep/pcapproval/lists");
