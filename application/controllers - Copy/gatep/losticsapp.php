...
class Losticsapp extends My_Controller {
    function __construct(){
        parent::__construct();
        $this->output->nocache();
        $this->load->model('gatep/losticsapp_model');
    ...
    function lists(){
      ...
      if($this->input->post('perpage')!='') $perpage=$this->input->post('perpage'); else $perpage=10;
        ...
        $config['base_url']=base_url().'gatep/losticsapp/lists/';
        $config['suffix'] = '?' . http_build_query($_GET, '', "&");
        $config["uri_segment"] = 4;
        $config['total_rows'] = $this->losticsapp_model->get_count();
        ...
        //echo $data["page"]; exit();///////////
        $data['list']=$this->losticsapp_model->lists($config["per_page"],$data['page'] );
        ////////////////////////////////////////
        $data['heading']='Gate Pass Lists';
        $data['display']='gatep/losticsapplist';
  ...
  function viewapproved($gatepass_id){
      if ($this->session->userdata('user_id')) {
        ...
        $data['display']='gatep/losticsappview';
        $this->load->view('admin/master',$data);
        } else {
           redirect("logincontroller");
        }
    ...
    function view($gatepass_id){
      ...
      public function approved($gatepass_id=FALSE){
        $data2['sub_contract_status']=2;
        $data2['approved_by2']=$this->session->userdata('user_id');
        $data2['approved_date2']=date('Y-m-d');
        ...
        redirect("gatep/losticsapp/lists");
