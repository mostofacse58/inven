...
class Mconfirm extends My_Controller {
    function __construct(){
        parent::__construct();
        $this->output->nocache();
        $this->load->model('gatep/mconfirm_model');
    ...
    function lists(){
      ...
      if($this->input->post('perpage')!='') $perpage=$this->input->post('perpage'); else $perpage=10;
        ...
        $config['base_url']=base_url().'gatep/mconfirm/lists/';
        $config['suffix'] = '?' . http_build_query($_GET, '', "&");
        $config["uri_segment"] = 4;
        $config['total_rows'] = $this->mconfirm_model->get_count();
        ...
        //echo $data["page"]; exit();///////////
        $data['list']=$this->mconfirm_model->lists($config["per_page"],$data['page'] );
        ////////////////////////////////////////
        $data['heading']='Gate Pass Lists';
        $data['display']='gatep/mconfirmlist';
  ...
  function viewapproved($gatepass_id){
      if ($this->session->userdata('user_id')) {
        ...
        $data['display']='gatep/mconfirmview';
        $this->load->view('admin/master',$data);
        } else {
           redirect("logincontroller");
        }
