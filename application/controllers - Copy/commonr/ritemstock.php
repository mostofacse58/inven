...
class Ritemstock extends My_Controller {
    function __construct(){
        parent::__construct();
        $this->load->model('commonr/ritemstock_model');
     }
    function SearchForm(){
        if ($this->session->userdata('user_id')) {
        $data['heading']='Re-order Label Stock Report';
		$data['clist']=$this->look_up_model->getCategory(12);
        $data['dlist']=$this->look_up_model->departmentList();
	    $data['display']='commonr/ritemstock';
    ...
    function reportrResult(){
       if ($this->session->userdata('user_id')) {
        ...
        $data['resultdetail']=$this->ritemstock_model->reportrResult($category_id,$rack_id,$data['box_id'],$department_id,$product_code);
        $data['display']='commonr/ritemstock';
        $this->load->view('admin/master',$data);
        }else{
        $data['display']='commonr/ritemstock';
    ...
    function safetyitem(){
      ...
      $data['rack_id']='All';
      $data['box_id']='All';
      $data['resultdetail']=$this->ritemstock_model->safetyitem($department_id);
      $data['display']='commonr/ritemstock';
      $this->load->view('admin/master',$data);
    }
    ...
    function downloadExcel($category_id=FALSE,$rack_id=FALSE,$box_id=FALSE,$department_id=FALSE,$product_code=FALSE) {
        ...
        $data['resultdetail']=$this->ritemstock_model->reportrResult($category_id,$rack_id,$box_id,$department_id,$product_code);
        $this->load->view('commonr/ritemstockExcel',$data);
    }
    function getBox(){
        $department_id=$this->session->userdata('department_id');
        $rack_id=$this->input->post('rack_id');
   ...
   function downloadPdf($category_id=FALSE,$rack_id=FALSE,$box_id=FALSE,$department_id,$product_code=FALSE) {
    if ($this->session->userdata('user_id')) {
      ...
      $html=$this->load->view('commonr/ritemstockPdf', $data, true);
