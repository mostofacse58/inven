...
class Itemstock extends My_Controller {
    ...
    function SearchForm(){
	    ...
	    $data['display']='commonr/itemstock';
     ...
     function loadExcel(){
        ...
        if($this->session->userdata('user_id')) {
            ...
            $data = $this->itemstock_model->loadExcel();        
    ...
    function reportrResult(){
       if ($this->session->userdata('user_id')) {
        ...
        $data['resultdetail']=$this->itemstock_model->reportrResult($category_id,$rack_id,$data['box_id'],$department_id,$product_code);
        $data['display']='commonr/itemstock';
        $this->load->view('admin/master',$data);
        }else{
        $data['display']='commonr/itemstock';
    ...
    function safetyitem(){
      ...
      $data['rack_id']='All';
      $data['box_id']='All';
      $data['resultdetail']=$this->itemstock_model->safetyitem($department_id);
      $data['display']='commonr/itemstock';
      $this->load->view('admin/master',$data);
    }
    ...
    function downloadExcel($category_id=FALSE,$rack_id=FALSE,$box_id=FALSE,$department_id=FALSE,$product_code=FALSE) {
        ...
        $data['resultdetail']=$this->itemstock_model->reportrResult($category_id,$rack_id,$box_id,$department_id,$product_code);
        $this->load->view('commonr/itemstockExcel',$data);
    }
    function getBox(){
        $department_id=$this->session->userdata('department_id');
        $rack_id=$this->input->post('rack_id');
