...
class Stockin extends My_Controller {
    function __construct()  {
        parent::__construct();
        $this->load->model('wh/stockin_model');
    ...
    function lists(){
      ...
      //////////////////
        ...
        $config['base_url']=base_url().'wh/stockin/lists/';
        ...
        $data['waiting']=$this->stockin_model->waiting();
        $data['total']=$this->stockin_model->total();
        ////////////////////////////
        $data['heading']='Carton Information';
        $data['llists']=$this->stockin_model->getLocation();
        $data['display']='wh/stockinlist';
        $this->load->view('admin/master',$data);
       }

       function dashboard(){
        $data['waiting']=$this->stockin_model->waiting();
        $data['alreadyIn']=$this->stockin_model->alreadyIn();
        $data['heading']='Invoice Wise Information';
        $data['list']=$this->stockin_model->getGroupByInvoice();
        $data['display']='wh/inInvlist';
        $this->load->view('admin/master',$data);
       }
      function po($export_invoice_no){
        $data['waiting']=$this->stockin_model->waitingInv($export_invoice_no);
        $data['alreadyIn']=$this->stockin_model->alreadyInv($export_invoice_no);
        $data['heading']='PO Wise Information';
        $data['list']=$this->stockin_model->getGroupByPo($export_invoice_no);
        $data['display']='wh/inpo';
        $this->load->view('admin/master',$data);
      }
    ...
    function add(){
      ...
      $data['llists']=$this->stockin_model->getLocation();
      $data['display']='wh/addin';
      $this->load->view('admin/master',$data);
    }
    function edit($id){
      $data['heading']='Edit Carton Information';
      $data['info']=$this->stockin_model->get_info($id);
      $data['llists']=$this->stockin_model->getLocation();
      $data['display']='wh/addin';
      $this->load->view('admin/master',$data);
    }
    function save($id=FALSE){
        ...
        $this->form_validation->set_rules('export_invoice_no','Invocie No','trim');
         if ($this->form_validation->run() == TRUE) {
            $check=$this->stockin_model->save($id);
             if($check && !$id){
    ...
    function delete($id=FALSE){
      $check=$this->stockin_model->delete($id);
        if($check){ 
