    ...
    <form class="form-horizontal" action="<?php echo base_url(); ?>payment/applications/save<?php if (isset($info)) echo "/$info->payment_id"; ?>" method="POST" enctype="multipart/form-data" onsubmit="return formsubmit();">
      ...
      <div class="form-group">
        ...
        <tr>
          ...
          <th style="width:35%;text-align:center">Bill Description</th>
          <th style="width:20%;text-align:center;">Amount</th>
          <th style="width:35%;text-align:center;">Remarks
            </th>
        ...
        <tbody>
         ...
         <table class="table table-bordered">
          ...
          <tr>
            ...
            <td style="width:35%;text-align:center">Net Payment:</td>
      ...
      <div class="col-sm-5" style="text-align: right;margin-bottom:10px;">  
         ...
         $id=0;
          if(isset($detail1)):
            foreach ($detail1 as  $value){
              ...
              $str.='<td style="text-align:left"><select name="dcode[]" onchange="return checkdepart(' . $id . ');" class="form-control select2"  style="width:100%;" id="dcode_' . $id . '"> '.$optionTree.'</select> </td> ';
              $str.= '<td><input type="text" onfocus="this.select()"  name="percentage[]" class="form-control integerchk" placeholder="percentage"  onblur="return DcalSum2(' . $id . ');" onkeyup="return DcalSum2(' . $id . ');"  value="'.$value->percentage.'"   style="margin-bottom:5px;width:98%;text-align:center" id="percentage_'.$id.'"/> </td>';
              $str.= '<td><input type="text" onfocus="this.select()"  name="damount[]" class="form-control integerchk" placeholder="Amount"  onblur="return DcalSum(' . $id . ');" onkeyup="return DcalSum(' . $id . ');"  value="'.$value->damount.'"   style="margin-bottom:5px;width:98%;text-align:center" id="damount_'.$id.'"/> </td>';
        ...
        <tr>
            ...
            <td style="width:20%;text-align:center;">
              <input type="text" name="dtotal_percentage" id="dtotal_percentage" readonly class="form-control" style="text-align: center;" value="<?php if(isset($info)) echo $info->dtotal_percentage; else echo 0; ?>">
            </td>
            <td style="width:20%;text-align:center;">
...
$("#AddManualItem1").click(function(){
  ...
  for(var i=0;i<id1;i++){
    if(deletedRow1.indexOf(i)<0) {
     ...
     perTotal += parseFloat($.trim($("#percentage_" + i).val()));
    }
  }
  var sub_total=parseFloat($("#sub_total").val());
  var rstofamount=sub_total-subTotal;
  var rstofpercentage=100-perTotal;
  ...
  var nodeStr = '<tr id="row1_' + id1 + '"><td  style="text-align:center"><b></b></td>'+
    ...
    '<td><input type="text" onblur="return DcalSum2(' + id1 + ');" onkeyup="return DcalSum2(' + id1 + ');" onfocus="this.select()"  name="percentage[]" value="' + rstofpercentage + '" class="form-control integerchk"  placeholder="percentage" style="width:98%;float:left;text-align:center" id="percentage_' + id1 + '"></td>' +
    '<td><input type="text" onblur="return DcalSum(' + id1 + ');" onkeyup="return DcalSum(' + id1 + ');" onfocus="this.select()"  name="damount[]" value="' + rstofamount + '" class="form-control integerchk"  placeholder="Amount" style="width:98%;float:left;text-align:center" id="damount_' + id1 + '"></td>' +
...
function TcalSum(){
  ...
  function departmentSum(){
    ...
    $("#dtotal_percentage").val(perTotal.toFixed(2));
  ////////////////
}
  function DcalSum(id2){
    ...
    var per_percenatnge=100/sub_total;
    var onedamount=parseFloat($.trim($("#damount_" + id2).val()));
    var percentage=per_percenatnge*onedamount;
    $("#percentage_"+id2).val(percentage);
    ///////////////////////
    var subTotal=0;
    var perTotal=0;
    var serviceNum=$("#form-table1 tbody tr").length;
    ...
    if(subTotal>sub_total){
        ...
        $("#alertMessagemodal").modal("show");
        $("#damount_"+id2).val(0);
        $("#percentage_"+id2).val(0);
    }
    departmentSum();
  }
  function DcalSum2(id2){
    var sub_total=parseFloat($("#sub_total").val());
    if($.trim(sub_total)==""|| typeof sub_total === "undefined"){
      sub_total=0;
    }
    var onedper=parseFloat($.trim($("#percentage_" + id2).val()));
    if(onedper>100){
        $("#alertMessageHTML").html("Percentage can not exceed 100 %!!!");
        $("#alertMessagemodal").modal("show");
        $("#damount_"+id2).val(0);
        $("#percentage_"+id2).val(0);
    }
    var per_amount=sub_total/100;
    var amountd=per_amount*onedper;
    $("#damount_"+id2).val(amountd);
    ///////////////////////
    var subTotal=0;
    var perTotal=0;
    var serviceNum=$("#form-table1 tbody tr").length;
