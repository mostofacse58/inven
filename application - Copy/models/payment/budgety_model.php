<?php
class Budgety_model extends CI_Model {
    function lists() {
      $condition=' ';
      if($_GET){
    ...
    function getLastMonth($head_id,$for_month){
      ...
      $row=$this->db->query("SELECT IFNULL(amount2,0) as amount
        FROM budget_yearly_details b ,budget_yearly_master a
      ...
      $department_id=$this->session->userdata('department_id');
        ...
        if($master_id==FALSE){
          $counts=$this->db->query("SELECT MAX(master_id) as counts 
            FROM budget_yearly_master 
            WHERE 1")->row('counts');
          $data['budget_no']='BDB'.str_pad($counts + 1, 6, '0', STR_PAD_LEFT);
          $query=$this->db->insert('budget_yearly_master',$data);
          ...
          $this->db->delete('budget_yearly_details');
        }
        $i=0;
        /////////////////////////////////////////
        ...
        foreach ($head_id as $value) {
          ...
          $query=$this->db->insert('budget_yearly_details',$data2);
        ...
        $this->db->WHERE('master_id',$master_id);
        $this->db->WHERE('by_department',$department_id);
        $query=$this->db->UPDATE('budget_yearly_master',$data3);
      return $master_id;
    }
   function deletes($master_id) {
     $department_id=$this->session->userdata('department_id');
      $this->db->WHERE('master_id',$master_id);
      $this->db->WHERE('department_id',$department_id);
      $query=$this->db->delete('budget_yearly_details');
      $this->db->WHERE('master_id',$master_id);
      $this->db->WHERE('by_department',$department_id);
      $query=$this->db->delete('budget_yearly_master');
      return $query;
    }
