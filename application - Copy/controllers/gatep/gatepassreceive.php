...
class Gatepassreceive extends My_Controller {
    function __construct(){
        parent::__construct();
        $this->output->nocache();
        $this->load->model('gatep/gatepassreceive_model');
    ...
    function lists(){
      ...
      if($this->input->post('perpage')!='') $perpage=$this->input->post('perpage'); else $perpage=10;
        ...
        $config['base_url']=base_url().'gatep/gatepassreceive/lists/';
        $config['suffix'] = '?' . http_build_query($_GET, '', "&");
        $config["uri_segment"] = 4;
        $config['total_rows'] = $this->gatepassreceive_model->get_count();
        ...
        //echo $data["page"]; exit();///////////
        $data['list']=$this->gatepassreceive_model->lists($config["per_page"],$data['page'] );
        ////////////////////////////////////////
        $data['heading']='Gate Pass Receive Lists';
        $data['display']='gatep/receivedlist';
  ...
  function viewreceived($gatepass_id){
      if ($this->session->userdata('user_id')) {
        ...
        $data['display']='gatep/receivedview';
        $this->load->view('admin/master',$data);
        } else {
           redirect("logincontroller");
        }
    ...
    function view($gatepass_id){
      ...
      public function received($gatepass_id=FALSE){
        $data2['gatepass_status']=7;
        ...
        redirect("gatep/gatepassreceive/lists");
